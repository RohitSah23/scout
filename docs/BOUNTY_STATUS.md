...
| ENS — Best Use of ENSv2, $4,500 | **Partial; funding-gated** | The runtime discovers the live resolver on every write, verifies forward resolution, reads actual key-scoped EAC roles and supports real authorized/reverted transactions from separate Privy wallets. A complete ENSv2 registration and EAC provisioning script uses the official Sepolia beta contracts. | Fund the generated owner wallet with Ethereum Sepolia ETH, run provisioning, preserve the resulting transaction proof, and make the ENS-resolved MCP endpoint/budget central to runtime behavior. |
